...
void Renderer::LoadTexture(const std::string & filename) {
  ...
  if (((width & (width - 1)) != 0) || ((height & (height - 1)) != 0)) {
    std::cerr << "Obrazek " + filename 
                 + " ma nieprawid³owe wymiary (powinny byæ potêg¹ 2): "
              << width << "x" << height << "\n";
    exit(1);
  }
...
void Renderer::DrawSprite(double tex_x, double tex_y, 
                          ...
                          size_t level) {
  const double texture_w = 256.0;    // szerokoœæ atlasu
  const double texture_h = 128.0;    // wysokoœæ atlasu
  ...
  /* Obrazek ³adowany jest do góry nogami, wiêc punkt (0,0) 
